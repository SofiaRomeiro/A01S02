...
void treeAdd(tree_node_s root, char buffer[]) {  
    ...
    for (i=0; i < len; i++) {
        if ((c = buffer[i]) == '/' || c == ' ' || c == '\0') {
            ...
            if (strcompare(directory,"") && strcompare(directory, "/")) {         
...
tree_node_s treeDelete(tree_node_s root, char buffer[]) {
    ...
    if (strlen(buffer) == 0) {
        ...
        if (!strcompare(aux->parent->child->path, aux->path)) {
            if (aux->brother != NULL)
            ...
            parent = aux->parent;
            finder = parent->child;
            while (strcompare(finder->brother->path, aux->path)) {
